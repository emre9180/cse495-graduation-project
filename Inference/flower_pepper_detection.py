...
for ix in range(len(rgb_images)):
    ...
    flower_results = model_flower(validation.rgb_path + "\\" + rgb_images[ix], save=True)
    ...
    flower_rbox, flower_allbox = postprocess(flower_results[0][0], flower_results[0][1], flower_results[0][2], impath=rgb_images[ix])
    ...
    prediction_dict[rgb_images[ix].split(".")[0]]["flower"] = flower_allbox
    ...
    try:
        ...
        predicts = validation.validate(prediction_dict, inference=False)
        ...
        results_path = r"C:\Users\emrey\OneDrive\Masaüstü\BitirmeFinal\yolov8\results_both\\"
